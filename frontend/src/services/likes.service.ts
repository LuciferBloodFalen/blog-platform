...
export class LikesService {
    ...
    static async getLikeStatus(postSlug: string): Promise<{ liked: boolean; likes_count: number }> {
        return await apiClient.get<{ liked: boolean; likes_count: number }>(`/posts/${postSlug}/like-status/`);
    }
}