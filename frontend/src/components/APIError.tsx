...
function parseError(error: Error | string): APIErrorDetails {
    ...
    // Network errors
    if (errorMessage.includes('NetworkError') || errorMessage.includes('Failed to fetch') || errorMessage.includes('ERR_NETWORK')) {
        return {
            type: 'network',
    ...
    // Validation errors
    if (errorMessage.includes('400') || errorMessage.includes('validation') || errorMessage.includes('Bad Request')) {
        return {
            type: 'validation',
...
export function APIErrorToast({ error, onClose, duration = 5000 }: APIErrorToastProps) {
    const errorDetails = parseError(error);
    ...
    return (
        <div className="fixed top-4 right-4 bg-red-50 border-l-4 border-red-500 p-4 rounded-lg shadow-lg max-w-md z-50">
