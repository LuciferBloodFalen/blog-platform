                ...
                <div className="p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500 mb-4 sm:mb-6">
                        <div className="flex items-center space-x-2 group/author">
                            ...
                            <span className="font-medium text-xs sm:text-sm truncate max-w-[100px] sm:max-w-none transition-colors duration-300 group-hover/author:text-black">{post.author}</span>
                        </div>
                        <span className="hidden sm:inline">•</span>
                        <span className="text-xs sm:text-sm">{formatDate(post.published_at || post.created_at)}</span>
                        {post.likes_count !== undefined && (
                            <>
                                <span className="hidden sm:inline">•</span>
                                <span className="flex items-center space-x-1 group/likes cursor-pointer">
                                    <svg className="w-3 h-3 sm:w-4 sm:h-4 transition-all duration-300 group-hover/likes:scale-125 group-hover/likes:text-red-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" />
                                    </svg>
                                    <span className="text-xs sm:text-sm transition-colors duration-300 group-hover/likes:text-red-500">{post.likes_count}</span>
                                </span>
                            </>
                        )}
                        {post.comments_count !== undefined && (
                            <>
                                ...
                                <span className="flex items-center space-x-1 group/comments cursor-pointer">
                                    <svg className="w-3 h-3 sm:w-4 sm:h-4 transition-all duration-300 group-hover/comments:scale-125 group-hover/comments:text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                    </svg>
                                    <span className="text-xs sm:text-sm transition-colors duration-300 group-hover/comments:text-blue-500">{post.comments_count}</span>
                                </span>
                            </>
                        )}
                    </div>
